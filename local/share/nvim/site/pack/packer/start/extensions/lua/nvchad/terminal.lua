...
M.show_term = function(term)
   ...
   if term.type ~= "float" then
      ...
      vim.api.nvim_open_win(0, true, calc_float_opts(chadterms.location.float))
   end
...
local config_handler = function(config)
   ...
   chadterms.winsize["horizontal"] = config.window.split_ratio or 0.5
   chadterms.winsize["vertical"] = config.window.vsplit_ratio or 0.5
   for k, v in pairs(config.location) do
      chadterms.location[k] = v
   end
end
